...
| **Cenas de motor** | SCD (função por sala) | ⚠ **mecanismo resolvido, conteúdo não**: cinemática é função de script (câmera+ator+espera+fade), e o opcode de disparo de AOT explica as **6 portas de caixa zerada** (são reais, disparadas por script). **135 gatilhos em 58 salas**; só o R10D está implementado. Ver [blueprints/08](blueprints/08_cenas_efeitos_e_travessia.md) |
| **Efeitos de sala** (fogo/fumaça) | RDT `off[17]` + pack HD | ✅ **156 salas** declaram efeito (até 8 ids); **71 salas / 154 pares / 1.386 quadros em HD** casados por vínculo geométrico; chaves de profundidade da Ordering Table medidas (posicionamento em 3D). Ver [blueprints/08](blueprints/08_cenas_efeitos_e_travessia.md) |
| **Travessia vertical** | RDT (assinatura de colisão) | 🟡 **degrau escalável é COLISÃO**, não objeto: plataforma de 1 nível + bloco `forma 8` no nível de cima → **28 registros em 14 salas** (inclui R504/R510). Hipótese forte, não provada no EXE |
| **Áudio de mundo** | `.ARD` + PC | ✅ **trilha por sala 169/169 medida**; cada sala embute o **banco de som** dela (48 ids, 168/169); **155 pontos de pedido de SE mapeados** (64 provados) = mapa de eventos de mundo |
