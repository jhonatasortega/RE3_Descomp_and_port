...
/**
 * Gera os segmentos de uma parede real (com vãos de porta recortados),
 * a partir de uma entrada de room.geom.json.walls[i].
 * Em vez de UMA caixa sólida cobrindo o vão, gera: segmento antes da porta,
 * segmento depois da porta, e uma verga (header) acima do vão — assim a
 * porta realmente "fura" a parede, em vez da sala virar um bloco fechado.
 */
function wallMeshes(wall, floorY) {
  const [ax, az] = wall.a
  const [bx, bz] = wall.b
  const length = wall.length || Math.hypot(bx - ax, bz - az) || 1
  const dirX = (bx - ax) / length
  const dirZ = (bz - az) / length
  // Alinha o eixo local X da caixa com a direção da parede (rotação em Y).
  const angle = Math.atan2(-dirZ, dirX)
  const thickness = Math.max(wall.thickness || 300, 60)
  const yBase = wall.y ?? floorY
  const wallH = Math.max(wall.height || 3000, 1)

  const openings = (wall.openings || [])
    .map(o => {
      const center = (o.u ?? 0.5) * length
      const w = Math.max(o.width || 1000, 1)
      return {
        start: Math.max(0, center - w / 2),
        end: Math.min(length, center + w / 2),
  ...
  const group = new THREE.Group()
  group.name = 'wallSeg'
  group.userData = { kind: 'wallSeg', fromRect: wall.from_rect }

  // segStart/segEnd em unidades ao longo da parede (0..length)
  // yOffset/hRaw em unidades PS1 verticais relativas a yBase (0 = piso da parede)
  function addSeg(segStart, segEnd, yOffset, hRaw) {
    const segLen = segEnd - segStart
    if (segLen <= 1 || hRaw <= 1) return
    const midT = (segStart + segEnd) / 2
    const midX = ax + dirX * midT
    const midZ = az + dirZ * midT
    const bottomGodot = M(-yBase) + M(yOffset)
    const hGodot = M(hRaw)

    const mesh = new THREE.Mesh(GEO.box, MATS.shell.clone())
    mesh.scale.set(M(segLen), hGodot, M(thickness))
    mesh.position.set(M(midX), bottomGodot + hGodot / 2, M(midZ))
    mesh.rotation.y = angle
    mesh.userData = { kind: 'shell', part: 'wall' }
    group.add(mesh)
  }

  let cursor = 0
  for (const o of openings) {
    if (o.start > cursor) addSeg(cursor, o.start, 0, wallH)
    if (o.sill > 0) addSeg(o.start, o.end, 0, o.sill) // trecho de parede abaixo do vão (peitoril)
    const doorTop = o.sill + o.height
    if (doorTop < wallH) addSeg(o.start, o.end, doorTop, wallH - doorTop) // verga acima da porta
    cursor = Math.max(cursor, o.end)
  }
  if (cursor < length) addSeg(cursor, length, 0, wallH)
...
/**
 * Constrói o Group THREE.js de uma sala a partir de room.src.json (+ room.geom.json).
 * @param {object} src  - conteúdo de room.src.json
 * @param {object} geom - conteúdo de room.geom.json (paredes reais); opcional
 * @param {object} pose - { tx, tz, rot_deg }
 * @param {boolean} opts.showMaskPlanes
 * @returns {THREE.Group}
 */
export function buildRoom(src, geom, pose = {}, opts = {}) {
  const id = src.room
  ...
  const hasRealWalls = geom && Array.isArray(geom.walls) && geom.walls.length > 0

  if (hasRealWalls && b.aabb) {
    // Paredes reais com vãos de porta, em vez de uma caixa única fechada.
    for (const wall of geom.walls) {
      shellGroup.add(wallMeshes(wall, floorY))
    }
  } else if (b.aabb) {
    // Fallback (sem room.geom.json): caixa única, como antes.
  ...
  // Piso e teto — planos separados (recebem a projeção da foto também).
  if (b.aabb) {
    const [x0, z0, x1, z1] = b.aabb
    const w = Math.max(x1 - x0, 1)
    const d = Math.max(z1 - z0, 1)
    ...
    const ceilingMesh = new THREE.Mesh(GEO.plane, MATS.ceiling.clone())
    ceilingMesh.rotation.x = Math.PI / 2
    ceilingMesh.scale.set(M(Math.max(w, 1)), M(Math.max(d, 1)), 1)
    ceilingMesh.position.set(cx, M(-floorY) + M(h) + 0.01, cz)
    ceilingMesh.name = 'ceiling'
    ceilingMesh.userData = { roomId: id, kind: 'ceiling' }
  ...
  // ── Colisão ──────────────────────────────────────────────────────────────
  // ATENÇÃO: estes retângulos são volumes de COLISÃO/GAMEPLAY do jogo original
  // (paredes de física, bloqueadores de móveis, sentinelas de IA) — não são a
  // arquitetura visual da sala, por isso não formam um contorno contínuo e
  // parecem "blocos soltos" quando ligados por cima da casca. Ficam ocultos
  // por padrão (ver layers.js); são úteis como referência para navegação/
  // colisão, não como parede a ser fotografada.
  const rectsGroup = new THREE.Group()
  rectsGroup.name = 'rects'
  rectsGroup.visible = false
