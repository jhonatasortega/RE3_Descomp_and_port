...
export function applyProjection(shellGroup, cameras, texMap, fovDeg = 55, opts = {}) {
  ...
  const shaderMat = new THREE.ShaderMaterial({
    ...
    vertexShader,
    fragmentShader,
    // BUGFIX: era BackSide (só visível de dentro da caixa). Várias câmeras
    // originais do PS1 ficam posicionadas FORA do volume da sala (comum em
    // jogos de câmera fixa pré-renderizada — a câmera "atravessa" o teto/
    // parede pra enquadrar o cenário). Com BackSide, a caixa ficava
    // invisível bem na hora de comparar a projeção com a foto de fundo.
    // DoubleSide garante que dá pra ver e calibrar de dentro E de fora.
...
export function removeProjection(shellGroup) {
  const defaultMat = new THREE.MeshLambertMaterial({
    color: 0x8a90a0, side: THREE.DoubleSide, // ver nota em applyProjection()
  })
  shellGroup.traverse(child => {
    if (child.isMesh && child.userData.kind === 'shell') {
