...
const state = {
  ...
  texMap: new Map(),
  projEnabled: true,
  bgPlane: null,  // plano de referência 3D com a foto (modo 'original')
}
...
const nav = new Navigator({
  onStageChange: async (data) => {
    ...
    // Seleciona a primeira sala automaticamente: como agora só a sala ativa
    // fica visível por padrão (ver layers.js), sem isso a cena apareceria
    // vazia até o usuário clicar em alguma sala na lista.
    if (data.rooms?.length) {
      await nav.selectRoom(data.rooms[0].id)
    }
...
async function loadStage(data) {
  ...
  // Camadas: agora que todas as salas do stage estão carregadas, o
  // LayerManager decide quais ficam visíveis (só a ativa, por padrão —
  // ver layers.js / toggle "Outras salas").
  layers.setAllRoomGroups(state.allRoomGroups)
...
async function loadRoom(data) {
  ...
  if (!state.allRoomGroups.has(src.room)) {
    const group = buildRoom(src, data.geom, pose)
  ...
  // Plano de referência com a foto (não mais scene.background em tela cheia:
  // isso escondia a geometria 3D projetada, que é o que precisa ser
  // calibrado/comparado com a foto).
  updateBgPlane()
...
// ── Plano de referência (foto) no modo Câmera Original ────────────────────────
// Substitui o antigo scene.background em tela cheia: agora a foto vira um
// plano 3D posicionado no mundo, atrás do shell projetado, para que dê pra
// comparar visualmente a geometria com a foto original ao mesmo tempo.
function updateBgPlane() {
  if (state.bgPlane) {
    scene.scene.remove(state.bgPlane)
    state.bgPlane = null
  }
  if (scene.mode !== 'original' || !state.currentRoom || !state.roomGroup) return

  const cams = state.currentRoom.room?.cameras || []
  const cam = cams[scene._camIndex]
  const tex = state.texMap.get(scene._camIndex)
  if (!cam || !tex) return

  const fov = parseFloat(document.getElementById('fovSlider')?.value || 55)
  const plane = createBgPlane(cam, tex, fov, state.roomGroup)
  if (plane) {
